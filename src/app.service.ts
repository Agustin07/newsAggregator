...
export class AppService {
	...
	private formatDateExpReg = "^(?:(?:31(\\/|-|\\.)(?:0?[13578]|1[02]))\\1|(?:(?:29|30)(\\/|-|\\.)(?:0?[13-9]|1[0-2])\\2))(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$|^(?:29(\\/|-|\\.)0?2\\3(?:(?:(?:1[6-9]|[2-9]\\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\\d|2[0-8])(\\/|-|\\.)(?:(?:0?[1-9])|(?:1[0-2]))\\4(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$";
	...
	createRequestNYT(req : Request, res : Response){
		...
		if (this.validateOption(req, 'oncontent'))
			url.length>3  ?  url+='&fq=body:'+params.oncontent : url+='fq=body:'+params.oncontent;

		if (this.validateOption(req, 'onsection')){
			const beforeSection = url.length>3 ?  ( params.hasOwnProperty('oncontent') ? ' AND ' : '&fq=' ) : 'fq=';
		...
		if (this.validateDateOption(req, 'todate')) {
			...
			( url.length > 3 ? url+='&end_date='+todate : url+='end_date='+todate);
		}

		if (this.validateDateOption(req, 'fromdate')) {
			const fromdate = this.formatDate(params.fromdate as string, 'NYT');
			( url.length > 3 ? url+='&begin_date='+fromdate : url+='begin_date='+fromdate);
		}

		if (this.validateNumberOption(req, 'onpage'))
			( url.length > 3 ? url+='&page='+params.onpage : url+='page='+params.onpage);
	...
	createRequestTG(req : Request, res : Response){
		...
		if (this.validateOption(req, 'oncontent'))
			( url.length > 3 ? url+=' AND '+params.oncontent+'&query-fields=body' : url+='q='+params.oncontent+'&query-fields=body');

		if (this.validateOption(req, 'onsection'))
			( url.length > 3 ? url+='&section='+params.onsection : url+='section='+params.onsection);

		if (this.validateDateOption(req, 'todate')) {
			const todate = this.formatDate(params.todate as string, 'TG');
			( url.length > 3 ? url+='&to-date='+todate : url+='to-date='+todate);
		}

		if (this.validateDateOption(req, 'fromdate')) {
			const fromdate = this.formatDate(params.fromdate as string, 'TG');
			( url.length > 3 ? url+='&from-date='+fromdate : url+='from-date='+fromdate);
		}
	...
	validateOption(req : Request, name : string){
		...
		if (params.hasOwnProperty(name)){
			if(params[name]==='') throw new Error(`Property "${name}" has no value!`);
			return true;
		}
		return false;
	}
	...
	validateNumberOption(req : Request, name : string){
		...
		if (params.hasOwnProperty(name)){
			...
			if ( !Number.isInteger(number) || number < 1 ) throw new Error(`Property "${name}" has not a valid integer number!`);
			return true;
		}
		return false;
	}
	...
	validateDateOption( req : Request, name : string){
		...
		if (params.hasOwnProperty(name)){
			...
			if (!regexp.test(params[name] as string)) throw new Error(`Property "${name}" has not a valid date!`);
			return true;
		}
		return false;
